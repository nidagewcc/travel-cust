 ...
 * 接口调用日志记录AOP实现
 * <p>
 * Created by Weishuo Zhang on 2016/11/3.
 */
@Component
@Aspect
public class LogAspect {

    private final static String REAL_IP_NAME = "X-Forward-For";
    ...
    @Before("execution(* com.phoenix.travel.controller..*.*(..))")
    public void doBeforeInServiceLayer(JoinPoint joinPoint) {
        startTimeMillis.set(System.currentTimeMillis());
    }

    @After("execution(* com.phoenix.travel.controller..*.*(..))")
    ...
    @Around("execution(* com.phoenix.travel.controller..*.*(..))")
    ...
    private void printOptLog() {
        ...
        log.info("\n############################ start ######################################\n" +
                ...
                endTimeMillis.get() - startTimeMillis.get(), JSON.toJSONString(inputParamMap.get()), JSON.toJSONString(result.get()));
